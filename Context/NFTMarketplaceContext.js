...
const projectId ="2WXIGqwRRxmKU6kcQoeZ5007V4b";
const projectSecretKey = "1516a0f2eaccb06a594292414d1578f4";
const auth = `Basic ${Buffer.from(`${projectId}:${projectSecretKey}`).toString(
  "base64"
)}`;  

const subdomain = "https://jayasatwik.infura-ipfs.io";
...
export const NFTMarketplaceProvider = ({ children }) => {
  ...
  const createSale = async (url, formInputPrice, isReselling, id) => {
    ...
    const listingPrice = ethers.utils.parseUnits("0.000025", "ether"); 
      ...
      const items = await Promise.all(
        data.map(
          async ({ tokenId, seller, owner, price: unformattedPrice }) => {
            ...
            console.log("data"+data+"token uri"+tokenURI);
